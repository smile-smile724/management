...
<template>
    <div class="mainbox">
        <div id="material">
                ...
                || data.stat.toLowerCase().includes(show_index.toLowerCase()))
                .filter(data => !search
                ...
                style="width: 100%;margin-top:39px">
                    ...
                    label="指导老师">
                        <template slot-scope="scope">
                            <span>{{scope.row.teacher1}}{{scope.row.teacher2!=''?'，'+scope.row.teacher2:''}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column
                    prop="fund"
                    ...
                    label="审批结果">
                        <template slot-scope="scope">
                            <span  v-if="scope.row.stat == '待审批'" style="color:blue;" slot="reference">审核中</span>
                            <span  v-if="scope.row.stat == '审批不通过'" style="color:red;" slot="reference">已拒绝</span>
                            <span  v-if="scope.row.stat == '审批通过'" style="color:green;" slot="reference">已通过</span>
                        </template>
                    </el-table-column>
...
export default {
    ...
    methods:{
        getPro(){
            ...
            axios({
                method: 'get',
                url: this.$host+'/manage_platform/achievement_msg'
            })
            .then(response => {
