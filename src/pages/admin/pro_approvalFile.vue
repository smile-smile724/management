...
<template>
    <div class="mainbox">
        <div id="material">
                ...
                || data.project.state.toLowerCase().includes(show_index.toLowerCase()))
                .filter(data => !search
                || data.project.projectName.toLowerCase().includes(search.toLowerCase())
                || data.project.typeName.toLowerCase().includes(search.toLowerCase())
                ...
                style="width: 100%;margin-top:39px">
                    ...
                    <el-table-column
                    prop="teacherList[0].name"
                    label="指导老师">
                        <template slot-scope="scope">
                            <span>{{scope.row.teacherList[0].name}}{{scope.row.teacherList.length==2?'，'+scope.row.teacherList[1].name:''}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column
                    prop="project.state"
                    label="审批结果">
                        <template slot-scope="scope">
                            <span  v-if="scope.row.project.state == '待审批'" style="color:blue;" slot="reference">审核中</span>
                            <span  v-if="scope.row.project.state == '审批不通过'" style="color:red;" slot="reference">已拒绝</span>
                            <span  v-if="scope.row.project.state == '审批通过'" style="color:green;" slot="reference">已通过</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="操作">
                        <template slot-scope="scope">
                            ...
                            :disabled="scope.row.fileUrl.length==0">下载</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
                ...
                || data.project.projectName.toLowerCase().includes(search.toLowerCase())
                || data.project.typeName.toLowerCase().includes(search.toLowerCase())
                || data.studentList[0].name.toLowerCase().includes(search.toLowerCase())).length"
    ...
    methods:{
        getPro(){
            ...
            .then(response => {
                ...
                if(data.code==12){
                    this.tableData=data.info.filter(data => data.fileUrl.length!=0);
